...
// Prices reflect real index levels at last close (Jul 10, 2026) sourced from
// live market data, shown here in USD terms for a consistent, single-currency
// dashboard (Oanda itself denominates UK100 in GBP and DE30/DAX in EUR, and
// JP225 / SPX500 / US30 already in USD — the point values below match the
// live index levels either way).
const INDEX_DATA = {
  UK100: {
    label: "UK100",
    fullName: "FTSE 100",
...
const NEWS_ITEMS = [
  {
    id: 1,
    headline: "Fed signals patience as core inflation cools for a third month",
    source: "Reuters",
    time: "12m ago",
    tag: "SPX500",
    sentiment: "BULLISH",
    summary:
      "Officials struck a measured tone on rate cuts, but softer inflation prints have futures markets leaning toward easier policy into year-end.",
  },
  {
    id: 2,
    headline: "BOJ hints at gradual policy normalization, yen firms",
    source: "Bloomberg",
    time: "38m ago",
    tag: "JPN225",
    sentiment: "BEARISH",
    summary:
      "A stronger yen is pressuring export-heavy names on the Nikkei, with auto and electronics majors leading the pullback.",
  },
  {
    id: 3,
    headline: "Eurozone flash PMI beats expectations, DAX extends gains",
    source: "Handelsblatt",
    time: "1h ago",
    tag: "DAX",
    sentiment: "BULLISH",
    summary:
      "Manufacturing and services both surprised to the upside, easing recession concerns for Germany's largest exporters.",
  },
  {
    id: 4,
    headline: "UK retail sales unexpectedly contract in June",
    source: "Financial Times",
    time: "2h ago",
    tag: "UK100",
    sentiment: "BEARISH",
    summary:
      "Consumer-facing names on the FTSE slipped after spending data missed forecasts, reviving questions on household demand.",
  },
  {
    id: 5,
    headline: "Wall Street futures steady ahead of key earnings week",
    source: "CNBC",
    time: "3h ago",
    tag: "US30",
    sentiment: "NEUTRAL",
    summary:
      "Traders are holding positioning flat into a heavy slate of large-cap earnings, with implied volatility ticking modestly higher.",
  },
  {
    id: 6,
    headline: "ECB officials signal no rush to cut rates further this quarter",
    source: "Reuters",
    time: "4h ago",
    tag: "DAX",
    sentiment: "NEUTRAL",
    summary:
      "Commentary was broadly balanced, keeping European equity futures range-bound in early trade.",
  },
  {
    id: 7,
    headline: "Dollar index slips as broader risk appetite improves",
    source: "MarketWatch",
    time: "5h ago",
    tag: "MACRO",
    sentiment: "BULLISH",
    summary:
      "A softer greenback is providing a tailwind across major indices, particularly names with meaningful overseas revenue exposure.",
  },
  {
    id: 8,
    headline: "Geopolitical tensions rattle energy markets, indices cautious",
    source: "Associated Press",
    time: "6h ago",
    tag: "MACRO",
    sentiment: "BEARISH",
    summary:
      "Crude's sharp move higher is weighing on sentiment broadly, with traders trimming risk ahead of the weekend.",
  },
  {
    id: 9,
    headline: "Mega-cap chip rally lifts Nasdaq 100 to fresh session highs",
    source: "Bloomberg",
    time: "45m ago",
    tag: "NAS100",
    sentiment: "BULLISH",
    summary:
      "Continued AI infrastructure spending is powering semiconductor and cloud names, with the index brushing against record territory.",
  },
  {
    id: 10,
    headline: "CAC 40 drifts sideways as luxury names offset energy weakness",
    source: "Les Echos",
    time: "1h ago",
    tag: "FRA40",
    sentiment: "NEUTRAL",
    summary:
      "Gains in high-end consumer names are roughly balancing softer oil-linked stocks, keeping Paris in a tight range.",
  },
  {
    id: 11,
    headline: "ASX 200 extends advance as miners track firmer iron ore",
    source: "Australian Financial Review",
    time: "7h ago",
    tag: "AU200",
    sentiment: "BULLISH",
    summary:
      "Resource heavyweights are leading gains as commodity prices firm, with the big four banks also contributing modest support.",
  },
];
...
// Mock 4HR candles — illustrative only, not real market ticks. JPN225's most
// recent candle close is pinned to match its live dashboard price so the
// signal reads as "current." Other tickers intentionally have no candle feed
// here since the sentiment filter (see getQualification) disqualifies them
// before any technical pattern is ever checked.
const CANDLES_4H = {
  JPN225: [
    { time: "Mon 04:00", open: 68300, high: 68420, low: 68100, close: 68250 }, // Candle A (anchor)
    { time: "Mon 08:00", open: 68250, high: 68300, low: 67980, close: 68050 },
    { time: "Mon 12:00", open: 68050, high: 68120, low: 67750, close: 67900 }, // swing low
    { time: "Mon 16:00", open: 67900, high: 68150, low: 67850, close: 68100 },
    { time: "Mon 20:00", open: 68100, high: 68450, low: 68050, close: 68400 },
    { time: "Tue 00:00", open: 68400, high: 68700, low: 68350, close: 68557.73 }, // breakout / trigger
  ],
};
    ...
    return {
      ...
      reason:
        sentiment.bullish === 60
          ? "60% bullish probability — must be strictly above 60%"
          : `${sentiment.bullish}% bullish probability — below the 60% threshold`,
    };
  }
  if (bias === "BEARISH") {
    ...
    return {
      ...
      reason:
        sentiment.bearish === 60
          ? "60% bearish probability — must be strictly above 60%"
          : `${sentiment.bearish}% bearish probability — below the 60% threshold`,
    };
  }
...
const BIAS_THEME = {
  ...
  BEARISH: {
    ...
    glow: "shadow-[0_0_40px_-12px_rgba(251,113,133,0.35)]",
  },
  NEUTRAL: {
    text: "text-amber-300",
    bar: "bg-amber-300",
    ...
    glow: "shadow-[0_0_40px_-12px_rgba(252,211,77,0.3)]",
  },
};
...
const IMPACT_THEME = {
  ...
  MEDIUM: { dot: "bg-amber-300", text: "text-amber-300", label: "Medium" },
...
// ---------------------------------------------------------------------------
// Shared bits: donut, ticker, header, page nav
// ---------------------------------------------------------------------------
...
function TickerTape({ active, onSelect }) {
  const items = ORDER.map((key) => INDEX_DATA[key]);
        ...
        {track.map((d, i) => {
          ...
          return (
            <button
              ...
              className={`flex shrink-0 items-center gap-2 text-xs font-mono tracking-tight transition-colors ${
                d.label === active ? "text-slate-100" : "text-slate-500 hover:text-slate-300"
              }`}
            >
          ...
          return (
            <button
              ...
              className={`flex flex-1 items-center justify-center gap-1.5 rounded-xl py-2 text-xs font-semibold transition-all ${
                isActive ? "bg-slate-800 text-slate-100 shadow-sm" : "text-slate-500 hover:text-slate-300"
              }`}
            >
              <Icon size={14} />
        ...
        {ORDER.map((key) => {
          ...
          return (
            <button
              ...
              className={`flex shrink-0 flex-col items-start gap-0.5 rounded-2xl border px-4 py-2 transition-all ${
                isActive ? "border-slate-700 bg-slate-900 shadow-sm" : "border-transparent bg-slate-900/40 hover:bg-slate-900/70"
              }`}
            >
              <span className={`text-xs font-semibold tracking-tight ${isActive ? "text-slate-100" : "text-slate-400"}`}>
                {d.label}
              </span>
    ...
    <div className="mx-auto max-w-2xl px-4 pb-8">
      ...
      <section className={`rounded-2xl border border-slate-800/80 bg-gradient-to-b from-slate-900 to-slate-900/60 p-5 shadow-xl ${theme.glow}`}>
        <div className="flex items-center justify-between">
          ...
          <span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${theme.border} ${theme.softBg} ${theme.text}`}>
            {data.confidence}% confidence
          </span>
        </div>

        <div className="mt-2 flex items-center gap-3">
          <span className={`flex h-11 w-11 items-center justify-center rounded-xl ${theme.softBg} ${theme.text}`}>
            <BiasIcon size={22} strokeWidth={2.5} />
          </span>
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <div className="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-4 shadow-sm">
          ...
          <div className="mt-2 font-mono text-lg font-semibold tabular-nums text-slate-100">
            {signed(data.range.low, 0)} <span className="text-slate-600">to</span> {signed(data.range.high, 0)}
          </div>
        ...
        <div className="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-4 shadow-sm">
          ...
          <div className="mt-2 flex items-center gap-2">
            <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-bold tracking-wide ${VOL_THEME[data.volatility]}`}>
              <Zap size={12} />
              {data.volatility}
            </span>
          </div>
          <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-slate-800">
            <div
              className={`h-full rounded-full ${data.volatility === "HIGH" ? "bg-rose-400" : data.volatility === "MEDIUM" ? "bg-amber-300" : "bg-emerald-400"}`}
              style={{ width: `${data.volScore}%` }}
            />
          </div>
      ...
      <div className="mb-4 flex items-center gap-2 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {filters.map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-semibold tracking-tight transition-colors ${
              filter === f
                ? "border-slate-700 bg-slate-800 text-slate-100"
                : "border-slate-800/80 bg-slate-900/40 text-slate-500 hover:text-slate-300"
            }`}
          >
          ...
          return (
            <article key={n.id} className="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-2">
                  <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${theme.softBg} ${theme.text}`}>
                    <Icon size={14} strokeWidth={2.5} />
                  </span>
                  <span className="rounded-full border border-slate-700/80 bg-slate-800/60 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400">
                    {n.tag === "MACRO" ? "Macro" : n.tag}
                  </span>
                </div>
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-8">
            ...
            <div key={group.day}>
              ...
              <div className="overflow-hidden rounded-2xl border border-slate-800/80 bg-slate-900/60 shadow-sm">
                {events.map((e, i) => (
                  <div
                    key={`${group.day}-${i}`}
                    className={`flex items-center gap-3 px-4 py-3 ${i !== events.length - 1 ? "border-b border-slate-800/60" : ""}`}
                  >
                    ...
                    <span className="w-11 shrink-0 rounded-md bg-slate-800/80 px-1.5 py-0.5 text-center text-[10px] font-bold text-slate-300">
                      {e.currency}
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[13px] font-medium text-slate-200">{e.event}</p>
                    </div>
                    <div className="hidden shrink-0 gap-3 text-right font-mono text-[11px] tabular-nums text-slate-500 sm:flex">
                      ...
                      <span className={e.actual !== "—" ? "font-semibold text-slate-200" : ""} title="Actual">
                        A {e.actual}
                      </span>
                    </div>
                  </div>
      ...
      <div className="mt-5 flex items-center justify-center gap-2 text-[11px] text-slate-600">
        <AlertCircle size={12} />
        Times shown in GMT · P = previous, F = forecast, A = actual
      </div>
    </div>
  );
}
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="rounded-full bg-slate-100 px-2 py-0.5 font-mono text-[10px] font-bold tabular-nums text-slate-900 shadow">
          {fmtPrice(price)}
        </span>
      </div>
    </div>
...
function LevelsPage({ active }) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-8">
      ...
      <div className="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-4 pt-6 shadow-sm">
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <div className="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-4 shadow-sm">
          ...
          <span className="mt-1.5 block text-[10px] text-slate-500">
            {fmt(Math.abs(data.levels.r1 - data.price), 1)} pts away
          </span>
        </div>
        <div className="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-4 shadow-sm">
          ...
          <span className="mt-1.5 block text-[10px] text-slate-500">
            {fmt(Math.abs(data.price - data.levels.s1), 1)} pts away
          </span>
        </div>
      </div>
    </div>
...
// ---------------------------------------------------------------------------
// Signals page — Pine Script "Confirmed Local Highs & Lows" fractal, filtered
// by dashboard sentiment probability
// ---------------------------------------------------------------------------
...
function CandlestickMini({ candles, signal }) {
  ...
  const bodyWidth = Math.min(22, slot * 0.5);
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${width} ${height}`} className="overflow-visible">
      {candles.map((c, i) => {
        ...
        const isAnchor = i === signal.anchorIndex;
        const isSwing = i === signal.swingIndex;
        const isTrigger = i === signal.triggerIndex;
        ...
        return (
          <g key={i}>
            ...
            <rect
              ...
              width={bodyWidth}
              height={Math.max(2, Math.abs(yFor(c.open) - yFor(c.close)))}
              fill={color}
              opacity={isTrigger ? 1 : 0.85}
              stroke={isTrigger ? "#f8fafc" : "none"}
              strokeWidth={isTrigger ? 1.5 : 0}
              rx={2}
            />
            {isAnchor && (
              <text x={x} y={yFor(c.high) - 8} textAnchor="middle" fontSize="11" fontWeight="700" fill="#fcd34d">
                A
              </text>
            )}
            {isSwing && (
              <text x={x} y={yFor(c.low) + 16} textAnchor="middle" fontSize="11" fontWeight="700" fill="#94a3b8">
                SL
              </text>
            )}
            {isTrigger && (
              <text x={x} y={yFor(c.high) - 8} textAnchor="middle" fontSize="11" fontWeight="700" fill="#34d399">
                ▲
              </text>
            )}
          </g>
      ...
      {/* anchor body-top reference line */}
      <line
        x1={padX}
        x2={width - padX}
        y1={yFor(Math.max(candles[signal.anchorIndex].open, candles[signal.anchorIndex].close))}
        y2={yFor(Math.max(candles[signal.anchorIndex].open, candles[signal.anchorIndex].close))}
        stroke="#fcd34d"
        strokeDasharray="3 4"
        strokeWidth={1}
        opacity={0.5}
      ...
      <line
        x1={padX}
        x2={width - padX}
        y1={yFor(signal.stopLoss)}
        y2={yFor(signal.stopLoss)}
        stroke="#fb7185"
        strokeDasharray="3 4"
        strokeWidth={1}
        opacity={0.6}
...
function SignalCard({ tickerKey, timeframe }) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-semibold text-slate-300">
              {tickerKey} <span className="font-normal text-slate-500">· {data.fullName}</span>
            </div>
            <p className="text-[11px] text-slate-500">{qual.reason} — macro/sentiment conditions not met</p>
          </div>
        </div>
        <span className="shrink-0 rounded-full border border-slate-700/80 bg-slate-800/60 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-500">
          No trigger search
        </span>
      </div>
    );
  ...
  return (
    <div className={`rounded-2xl border bg-gradient-to-b from-slate-900 to-slate-900/60 p-4 shadow-xl ${dirTheme.border} ${dirTheme.glow}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className={`flex h-9 w-9 items-center justify-center rounded-xl ${dirTheme.softBg} ${dirTheme.text}`}>
        ...
        <span className={`flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs font-bold uppercase tracking-wide ${dirTheme.border} ${dirTheme.softBg} ${dirTheme.text}`}>
      ...
      {signal ? (
        <>
          <div className="mt-4 flex items-center gap-2">
            <span className={`flex h-11 w-11 items-center justify-center rounded-xl ${dirTheme.softBg} ${dirTheme.text}`}>
...
function SignalsPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-8">
      ...
      <div className="mt-5 flex items-center justify-center gap-2 text-center text-[11px] text-slate-600">
        <AlertCircle size={12} />
        Illustrative mock data and rules-based logic · not financial advice
      </div>
    </div>
  );
}
...
export default function TradeIndicesApp() {
  ...
  return (
    <div className="min-h-screen w-full bg-slate-950 text-slate-100 font-sans antialiased">
      ...
      <header className="sticky top-0 z-30 border-b border-slate-800/80 bg-slate-950/90 backdrop-blur">
        <div className="mx-auto flex max-w-2xl items-center justify-between px-4 py-4">
          <button
            ...
            className="rounded-xl p-2 text-slate-400 transition-colors hover:bg-slate-900 hover:text-slate-100"
          ...
          <div className="flex items-center gap-1.5 rounded-full border border-emerald-400/20 bg-emerald-400/5 px-2.5 py-1.5">
            ...
            <span className="text-[11px] font-medium uppercase tracking-widest text-emerald-400">Live</span>
          </div>
        </div>

        <PageNav page={page} setPage={setPage} />
        {showIndexTabs && <IndexTabs active={active} setActive={setActive} />}
      </header>
          ...
          <div className="absolute left-0 top-0 h-full w-72 border-r border-slate-800 bg-slate-950 p-5">
            ...
            <nav className="flex flex-col gap-1">
              {PAGES.map(({ key, label, icon: Icon }) => (
                <button
                  ...
                  className={`flex items-center justify-between rounded-xl px-3 py-2.5 text-sm transition-colors ${
                    page === key ? "bg-slate-900 text-slate-100" : "text-slate-300 hover:bg-slate-900 hover:text-slate-100"
                  }`}
                >
              ...
              {[
                ...
                <button
                  key={label}
                  className="flex items-center justify-between rounded-xl px-3 py-2.5 text-sm text-slate-300 transition-colors hover:bg-slate-900 hover:text-slate-100"
                >
                  <span className="flex items-center gap-3">
                    <Icon size={16} className="text-slate-500" />
                    {label}
      ...
      <main className="pt-5">
        {page === "dashboard" && <DashboardPage active={active} />}
        {page === "news" && <NewsPage />}
        {page === "calendar" && <CalendarPage />}
        {page === "levels" && <LevelsPage active={active} />}
        {page === "signals" && <SignalsPage />}

        <div className="mx-auto flex max-w-2xl items-center justify-center gap-2 px-4 pb-4 text-[11px] text-slate-600">
          <Radio size={12} />
          Prices shown in USD for consistency · static preview, not a live feed
        </div>
      </main>
    </div>
  );
